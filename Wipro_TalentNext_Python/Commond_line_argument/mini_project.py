...
'''
import sys 
def cal_h(likes,dislikes,num_check):
    happiness = 0
    for num_str in num_check :
        try : 
            number = int(num_str)
            if number in likes:
                happiness +=1
            ...
            print(f"Invalid number found : '{num_str}'")
            sys.exit(1)
...
try : 
        liked_num = set(map(int,sys.argv[1].split('-')))    
        disliked_num = set(map(int,sys.argv[2].split('-')))    
        num_check = sys.argv[3].split('-')    
except ValueError :
        print("only use integers separated by '-' ")
        sys.exit(1)
        
score = cal_h(liked_num, disliked_num,num_check)
print(score)   
